from CODE.transquest.training.run_multi import predict_MultiTransQuest, multitask_mixed_languages
...
def main():
    ...
    TASK_CONFIG = {
        ...
        'num_separate_decoder_layers': 2,
        'learning_rates': [2e-5, 2e-5],
        'train_batch_size': 16
    }

    # Training
    multitask_mixed_languages(languages=[language, "RO-EN"], label=label, is_sweeping=False,
    ...
    predictions, original_dataframe = predict_MultiTransQuest(language=language, model_language=language, dataset=val_set, save_results=False,
                            aux_type="bilingual", experiment="demo", evaluation_type=label, data='MLQE', task='regression',
                            task_config=TASK_CONFIG)
