...
# HANNA: Commented out to make it work with Colab
import sys
from CODE.transquest.algo.sentence_level.multitransquest.model_args import MultiTransQuestArgs
...
from CODE.transquest.algo.sentence_level.multitransquest.models.xlm_roberta_model import XLMRobertaForMultitaskSequenceClassification
from CODE.transquest.algo.sentence_level.multitransquest.utils import LazyClassificationDataset, InputExample, \
...
from CODE.transquest.algo.sentence_level.multitransquest.load_data import MultitaskSampler
...
class MultiTransQuestModel:
    def __init__(
        ...
        self.config.update({'num_tasks': self.args.num_tasks, 'list_labels': self.args.num_labels, 'grad_weights': self.args.grad_weights, \
                            'num_shared_decoder_layers':self.args.num_shared_decoder_layers, 'num_separate_decoder_layers':self.args.num_separate_decoder_layers, \
     ...
     def train_model(
        ...
        if args:
            ...
            assert len(self.args.grad_weights) == self.args.num_tasks
            assert len(self.args.task_weights) == self.args.num_tasks
        ...
        # Create random seed to allow shuffling while aligning source and target sentence
    ...
    def train(
        ...
        # When summing the losses, the numbers training steps correspond to #batches divided by #tasks
        t_total = (len(train_dataloader) // args.gradient_accumulation_steps * args.num_train_epochs) // args.num_tasks
        ...
        acc_train_loss, logging_loss = 0.0, 0.0
        model.zero_grad()
        ...
        current_loss_per_task = ["Initializing"] * self.num_tasks
        ...
        for _ in train_iterator:
            ...
            # List to store features when computing the encoder and decoder pass separately
            features = [None] * 2
            loss = [None] * args.num_tasks
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                # Pass the inputs separately through the encoder, compute encoded features
                if args.separate_encoder_pass:
                    inputs['encode_decode']='encode'
                    if self.args.fp16:
                        with amp.autocast():
                            features[curr_task] = model(**inputs)

                    else:
                        features[curr_task] = model(**inputs)

                # Behaviour when doing the entire pass at once
                else:
                    if self.args.fp16:
                        with amp.autocast():
                            outputs = model(**inputs)
                            # model outputs are always tuple in pytorch-transformers (see doc)
                            loss[curr_task] = outputs[0]
                    else:
                        outputs = model(**inputs)
                        # model outputs are always tuple in pytorch-transformers (see doc)
                        loss[curr_task] = outputs[0]
                    ...
                    if args.n_gpu > 1:
                        loss[curr_task] = loss[curr_task].mean()  # mean() to average on multi-gpu parallel training

                    current_loss_per_task[curr_task] = loss[curr_task].item()

                    if show_running_loss:
                        main_loss=current_loss_per_task[0]
                        batch_iterator.set_description(
                            f"Epochs {epoch_number}/{args.num_train_epochs}. Running Loss (main task): {main_loss:9.4f}"
                        )

                    if args.gradient_accumulation_steps > 1:
                        loss[curr_task] = loss[curr_task] / args.gradient_accumulation_steps


                if curr_task == args.num_tasks-1:


                    if args.separate_encoder_pass:
                        assert args.combine_feature_vectors in ['hadamard', 'sum', 'subtract']

                        if args.combine_feature_vectors == "hadamard":
                            combined_features = features[0] * features[1]

                        elif args.combine_feature_vectors == "sum":
                            combined_features = features[0] + features[1]

                        elif args.combine_feature_vectors == "subtract":
                            combined_features = features[0] - features[1]

                        else:
                            combined_features = features[1]


                        outputs0 = model.forward(curr_task=0, encode_decode='decode', encoded_features=combined_features, labels = batch[3].to(self.device))
                        outputs1 = model.forward(curr_task=1, encode_decode='decode',encoded_features=features[0], labels = batch[3].to(self.device))

                        loss[0] = outputs0[0]
                        loss[1] = outputs1[0]

                    # Sum task-specific losses and backpropagate them
                  
                    weighted_total_train_loss = sum([loss[i] * args.task_weights[i] for i in range(self.num_tasks)])

                    if self.args.fp16:
                        scaler.scale(weighted_total_train_loss).backward()
                    else:
                        weighted_total_train_loss.backward()

                    acc_train_loss += weighted_total_train_loss.item()
                    ...
                    if (step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                            ...
                            tb_writer.add_scalar("total_loss", (acc_train_loss - logging_loss) / args.logging_steps, global_step)
                            logging_loss = acc_train_loss
                            if args.wandb_project or self.is_sweeping:
                                wandb.log(
                                    {
                                        "train_loss_per_task": current_loss_per_task, #this is now a list
                                        "train_loss_total": weighted_total_train_loss,
                                        #"task_weights": args.task_weights, # list
                                        #"grad_weights": args.grad_weights, # list
                        ...
                        if args.evaluate_during_training and (
                            ...
                            for eval_task in range(self.num_tasks):
                                ...
                                if args.separate_encoder_pass:
                                    results, _, _ = self.eval_separate_pass_model(
                                        eval_df, curr_task=eval_task,
                                ...
                                training_progress_scores[eval_task]["train_loss"].append(current_loss_per_task[eval_task])
                                for key in results:
        ...
        return (
            global_step,
            acc_train_loss / global_step if not self.args.evaluate_during_training else training_progress_scores,
        )

    def eval_separate_pass_model(
            self, eval_df, curr_task, output_dir=None, verbose=True, silent=False, wandb_log=True,**kwargs
    ):
        if not output_dir:
            output_dir = self.args.output_dir
        ...
        eval_datasets = []
        for index, df in enumerate(eval_df):
            eval_examples = [
                InputExample(i, text, None, label)
            ...
            eval_dataset = self.load_and_cache_examples(
                eval_examples, curr_task=index, evaluate=True, verbose=verbose, silent=silent
            )

            eval_datasets.append(eval_dataset)

        eval_dataset_concat = ConcatDataset(eval_datasets)
        eval_sampler = MultitaskSampler(dataset = eval_dataset_concat, batch_size = self.args.eval_batch_size)
        ...
        preds = np.empty((len(eval_df[0]), self.num_labels_per_task[curr_task]))
        out_label_ids = np.empty((len(eval_df[0])))

        features = [None]*2
        #tmp_eval_loss = [None]*2
        #logits = [None] * 2
        for i, batch in enumerate(tqdm(eval_dataloader, disable=args.silent or silent, desc="Running Evaluation")):

            source_or_target = i % args.num_tasks
            ...
            with torch.no_grad():
                ...
                features[source_or_target] = model(**inputs)

                if source_or_target == args.num_tasks-1:
                    if curr_task == 0:

                        assert args.combine_feature_vectors in ['hadamard', 'sum', 'subtract']

                        if args.combine_feature_vectors == "hadamard":
                            task_features = features[0] * features[1]
                    ...
                    else: task_features = features[1]


                    outputs = model.forward(curr_task=curr_task, encode_decode='decode',
                                             encoded_features=task_features, labels=batch[3].to(self.device))
                    ...
                    start_index = self.args.eval_batch_size * ((i-1)/2)
                    end_index = start_index + self.args.eval_batch_size if i != (n_batches - 1) else len(eval_df[0])
                    start_index = int(start_index)
                    end_index = int(end_index)

                    preds[start_index:end_index] = logits.detach().cpu().numpy()[:end_index-start_index]

                    all_labels = eval_df[0]["labels"]

                    out_label_ids[start_index:end_index] = all_labels[start_index:end_index]



        eval_loss = eval_loss / nb_eval_steps

        if args.regression[curr_task] is True:
