...
class MultiTransQuestModel:
    def __init__(
        ...
        self.config.update({'num_tasks': self.args.num_tasks, 'num_shared_decoder_layers':self.args.num_shared_decoder_layers, \
                            'grad_weights': self.args.grad_weights, 'num_separate_decoder_layers':self.args.num_separate_decoder_layers, \
                            'list_labels': self.args.num_labels, 'id2label':None, 'label2id':None})
    ...
    def train(
        ...
        # Initialise optimizers (because of the momentum-like features of AdamW, each task needs a separate optimizer)
        optimizer_dict = {}
        scheduler_dict = {}
        for task in range(self.args.num_tasks):
            if args.optimizer == "AdamW":
                optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rates[task], eps=args.adam_epsilons[task])
            elif args.optimizer == "Adafactor":
                optimizer = Adafactor(
                    optimizer_grouped_parameters,
                    lr=args.learning_rates[task],
            ...
            # Store the optimizers and schedulers in a dictionary
            optimizer_dict[task] = optimizer
            scheduler_dict[task] = scheduler

        if args.n_gpu > 1:
            model = torch.nn.DataParallel(model)

        global_step = 0
        local_step = [0.0] * self.num_tasks
        training_progress_scores = None
        tr_loss =  [0.0] * self.num_tasks
        logging_loss =  [0.0] * self.num_tasks
        model.zero_grad()
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                current_loss_per_task[curr_task]  = loss.item()

                if show_running_loss:
                    batch_iterator.set_description(
                        f"Epochs {epoch_number}/{args.num_train_epochs}. Running Loss: {current_loss_per_task[0]:9.4f}"
                    )
                ...
                tr_loss[curr_task] += loss.item()
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    if self.args.fp16:
                        scaler.unscale_(optimizer_dict[curr_task])
                    if args.optimizer == "AdamW":
                        torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)

                    if self.args.fp16:
                        scaler.step(optimizer_dict[curr_task])
                        scaler.update()
                    else:
                        optimizer_dict[curr_task].step()
                    scheduler_dict[curr_task].step()  # Update learning rate schedule
                    model.zero_grad()
                    global_step += 1
                    local_step[curr_task] += 1
                    torch.cuda.empty_cache()

                    if args.logging_steps > 0 and local_step[curr_task] % args.logging_steps == 0:
                        # Log metrics
                        tb_writer.add_scalar("lr_"+str(curr_task), scheduler_dict[curr_task].get_last_lr()[0], local_step[curr_task])
                        tb_writer.add_scalar("loss_"+str(curr_task), (tr_loss[curr_task] - logging_loss[curr_task]) / args.logging_steps, local_step[curr_task])
                        logging_loss[curr_task] = tr_loss[curr_task]
                        if args.wandb_project or self.is_sweeping:
                            for task in range(self.args.num_tasks):
                                wandb.log(
                                    {
                                        "training_loss_task_"+str(task): current_loss_per_task[task],
                                        "lr_task_"+str(task): scheduler_dict[task].get_last_lr()[0],
                                        "local_step_"+str(task): local_step[task],
                                    }
                                )
                    ...
                    if args.save_steps > 0 and global_step % args.save_steps == 0:
                        ...
                        self.save_model(output_dir_current, optimizer_dict, scheduler_dict, model=model)
                    ...
                    if args.evaluate_during_training and (
                        ...
                        for eval_task in range(self.num_tasks):
                            ...
                            for key, value in results.items():
                                tb_writer.add_scalar("eval_task_"+str(eval_task)+"{}".format(key), value, local_step[eval_task])
                            ...
                            if args.save_eval_checkpoints:
                                if args.save_recent_only:
                                    ...
                                    for del_path in del_paths:
                                        shutil.rmtree(del_path)
                                self.save_model(output_dir_current, optimizer_dict, scheduler_dict, model=model, results=results)

                            training_progress_scores[eval_task]["local_step"].append(local_step[eval_task])
                            ...
                            if not best_eval_metric[eval_task]:
                                ...
                                self.save_model(args.best_model_dir, optimizer_dict, scheduler_dict, model=model, results=results)
                            if best_eval_metric[eval_task] and args.early_stopping_metric_minimize:
                                if best_eval_metric[eval_task] - results[args.early_stopping_metric] > args.early_stopping_delta:
                                    best_eval_metric[eval_task] = results[args.early_stopping_metric]
                                    self.save_model(
                                        args.best_model_dir, optimizer_dict, scheduler_dict, model=model, results=results
                                    )
                                    early_stopping_counter = 0
                                else:
                                    if args.use_early_stopping:
                                        if early_stopping_counter < args.early_stopping_patience:
                                            ...
                                            return (
                                                local_step[eval_task],
                                                tr_loss[eval_task] / local_step[eval_task]
            ...
            if args.evaluate_during_training and args.evaluate_each_epoch:
                ...
                for eval_task in range(self.num_tasks):
                    ...
                    self.save_model(output_dir_current, optimizer_dict, scheduler_dict, results=results)
        ...
        return (
            local_step[0],
            tr_loss[0] / local_step[0] if not self.args.evaluate_during_training else training_progress_scores,
        )
    ...
    def save_model(self, output_dir=None, optimizer=None, scheduler=None, model=None, results=None):
        ...
        if model and not self.args.no_save:
            ...
            if optimizer and scheduler and self.args.save_optimizer_and_scheduler:
                for task in range(self.args.num_tasks):
                    torch.save(optimizer[task].state_dict(), os.path.join(output_dir, "optimizer_"+[task]+".pt"))
                    torch.save(scheduler[task].state_dict(), os.path.join(output_dir, "scheduler_"+[task]+".pt"))
            self.save_model_args(output_dir)
